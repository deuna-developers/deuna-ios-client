...
[**expireOrder**](OrderAPI.md#expireorder) | **POST** /merchants/orders/{order_token}/expire | Expirar una orden
...
# **expireOrder**
```swift
    open class func expireOrder(orderToken: String, xApiKey: String? = nil, completion: @escaping (_ data: Void?, _ error: Error?) -> Void)
```
...
// Expirar una orden
OrderAPI.expireOrder(orderToken: orderToken, xApiKey: xApiKey) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
# **getOrder**
```swift
    open class func getOrder(orderToken: String, xApiKey: String? = nil, completion: @escaping (_ data: GetOrder200Response?, _ error: Error?) -> Void)
```
